    ...
    def get_action(self, obs: dict):
        ...
        for k, v in obs.items():
            ...
            if isinstance(v, torch.Tensor):
                ...
                info_dict[k] = torch.tensor(v).unsqueeze(0).to(next(self.jepa.parameters()).device)
...
def run(config: ModelConfig):
    # For evaluation, we typically want to specify a checkpoint path
    # This can be done via command line: ++ckpt_path=/path/to/ckpt
    ckpt_path = OmegaConf.to_container(config).get("ckpt_path", None)
    if not ckpt_path:
        print("Warning: No ckpt_path provided. Using randomly initialized model (for testing only).")
    ...
    wandb.init(project="SkillJEPA_Eval", name=run_name, config=OmegaConf.to_container(config, resolve=True))

    world_cfg = OmegaConf.to_container(config.world, resolve=True)
    world = swm.World(**world_cfg, image_shape=(config.vision.frame_size, config.vision.frame_size))
    ...
    process = {}
    # Use config-driven keys to cache/process
    ...
    # Solver instantiation using Hydra
    ...
    policy = AgnosticBufferedPolicy(solver=solver, jepa=jepa, process=process, transform=transform)
