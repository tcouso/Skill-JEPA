...
class StandardJEPA(nn.Module):
    ...
    def __init__(self, config: ModelConfig, encoder: nn.Module, predictor: nn.Module, projector: nn.Module = None, pred_proj: nn.Module = None):
        super().__init__()
        self.config = config
    ...
    def rollout(self, info: Dict[str, torch.Tensor], action_sequence: torch.Tensor, history_size: int = None) -> Dict[str, torch.Tensor]:
    ...
    def get_cost(self, info_dict: Dict[str, torch.Tensor], action_candidates: torch.Tensor) -> torch.Tensor:
        ...
        for k in list(info_dict.keys()):
            if k.startswith("goal_"): goal[k[len("goal_") :]] = goal.pop(k)
    ...
    def get_executable_actions(self, info_dict: Dict[str, torch.Tensor], best_sequence: torch.Tensor) -> torch.Tensor:
...
class SkillJEPA(nn.Module):
    ...
    def __init__(self, config: ModelConfig, encoder: nn.Module, predictor: nn.Module, action_encoder: nn.Module, projector: nn.Module = None, pred_proj: nn.Module = None):
        super().__init__()
        self.config = config
        self.encoder = encoder
        self.predictor = predictor
    ...
    def rollout(self, info: Dict[str, torch.Tensor], skill_sequence: torch.Tensor, history_size: int = None) -> Dict[str, torch.Tensor]:
        if history_size is None:
            history_size = self.config.history_size
        H = info["pixels"].size(2) if "pixels" in info else 1
    ...
    def get_executable_actions(self, info_dict: Dict[str, torch.Tensor], best_sequence: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            first_latent = best_sequence[0] if best_sequence.ndim == 2 else best_sequence
            first_latent = torch.tensor(first_latent, device=state_emb.device).unsqueeze(0)
